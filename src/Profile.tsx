...
export default function Profile() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      setUser(data.user)
      if (data.user) fetchMyListings(data.user.id)
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: 16, fontFamily: 'sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        ...
        <h2 style={{ margin: 0 }}>Omat ilmoitukset</h2>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
        <button
          onClick={() => goTo('#new')}
          style={{ background: '#1a1a2e', color: 'white', border: 'none', padding: '10px 20px', borderRadius: 8, cursor: 'pointer', fontWeight: 'bold' }}
        >
          + Uusi ilmoitus
        </button>
        <button
          onClick={() => goTo('#messages')}
          style={{ background: '#e63946', color: 'white', border: 'none', padding: '10px 20px', borderRadius: 8, cursor: 'pointer', fontWeight: 'bold' }}
        >
          ✉️ Viestit
        </button>
      </div>
              ...
              <div style={{ padding: 12, flex: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <span style={{
                    ...
                    background: listing.status === 'active' ? '#d4edda' : listing.status === 'sold' ? '#f8d7da' : '#fff3cd',
                    color: listing.status === 'active' ? '#2d6a4f' : listing.status === 'sold' ? '#c1121f' : '#856404'
                  }}>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
                  <button
                    onClick={() => goTo(`#edit-${listing.id}`)}
                    style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid #ddd', background: 'white', cursor: 'pointer', fontSize: 13 }}
                  >
                    ✏️ Muokkaa
                  </button>
                  {listing.status !== 'sold' && (
                    <>
                      ...
                      <button
                        ...
                        style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid #52b788', background: '#d4edda', cursor: 'pointer', fontSize: 13, color: '#2d6a4f' }}
                      >
                        ✅ Merkitse myydyksi
                      </button>
                    </>
                  ...
                  <button
                    ...
                    style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid #f8d7da', background: '#f8d7da', cursor: 'pointer', fontSize: 13, color: '#c1121f' }}
                  >
                    🗑 Poista
                  </button>
                </div>
